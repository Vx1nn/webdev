						...
						<table class="table table-hover table-striped mb-0">
							...
							<tbody>
								@forelse($data as $temu)
									<tr class="align-middle">
										...
										<td>
											<button class="btn btn-sm btn-warning" data-bs-toggle="modal"
												data-bs-target="#editModal{{ $temu->idtemu_dokter }}">
												<i class="bi bi-pencil"></i>
											</button>
											<button class="btn btn-sm btn-danger" data-bs-toggle="modal"
												data-bs-target="#deleteModal{{ $temu->idtemu_dokter }}">
									...
									<div class="modal fade" id="editModal{{ $temu->idtemu_dokter }}" tabindex="-1">
										<div class="modal-dialog">
											<div class="modal-content">
												<form action="{{ route('admin.temu-dokter.edit', $temu->idtemu_dokter) }}"
													method="POST">
													@csrf
													...
													<div class="modal-body">
														<div class="mb-3">
															<label class="form-label">Pet <span
														...
														<div class="mb-3">
															<label class="form-label">Waktu Daftar <span
																	class="text-danger">*</span></label>
															<input type="datetime-local" name="waktu_daftar"
																class="form-control"
																value="{{ $temu->waktu_daftar ? \Carbon\Carbon::parse($temu->waktu_daftar)->format('Y-m-d\TH:i') : '' }}"
																required>
														</div>
														<div class="mb-3">
														...
														<div class="mb-3">
															...
															<select name="status" class="form-control" required>
																<option value="0" {{ $temu->status == '0' ? 'selected' : '' }}>
																	Menunggu</option>
																<option value="1" {{ $temu->status == '1' ? 'selected' : '' }}>
																	Selesai</option>
															</select>
														</div>
													</div>
									...
									<!-- Delete Modal -->
									<div class="modal fade" id="deleteModal{{ $temu->idtemu_dokter }}" tabindex="-1">
										<div class="modal-dialog">
											<div class="modal-content">
												<form action="{{ route('admin.temu-dokter.delete', $temu->idtemu_dokter) }}"
													method="POST">
													@csrf
													<div class="modal-header">
														<h5 class="modal-title">Hapus Jadwal Temu Dokter</h5>
														<button type="button" class="btn-close"
															data-bs-dismiss="modal"></button>
													</div>
													<div class="modal-body">
														<p>Apakah Anda yakin ingin menghapus jadwal temu dokter untuk pet
					...
					<div class="modal-body">
						<div class="mb-3">
							...
							<select name="idpet" class="form-control" required>
								...
								@foreach($pets as $pet)
									<option value="{{ $pet->idpet }}">{{ $pet->nama }} ({{ $pet->pemilik->user->nama ?? '-' }})
									</option>
								@endforeach
							</select>
						</div>
