...
<div class="container">
  <div class="card shadow radius">
    <div class="card-header">Tambah Jenis Hewan Baru</div>
    <div class="card-body">
      @if ($errors->any())
      ...
      <form action="{{ route('admin.jenis-hewan.store') }}" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" id="nama_jenis" name="nama_jenis"
                 placeholder="Contoh: Kucing" value="{{ old('nama_jenis') }}" required>
        </div>

        <button type="submit" class="btn-login">Simpan</button>
        <a href="{{ route('admin.jenis-hewan.index') }}" class="btn-cancel">Batal</a>
      </form>
    </div>
  </div>
</div>
